...
module BattleCommand_Custom
  ...
  ACTOR_COMMAND_LIST = [:skill, :guard, :item]
  ...
  # 　スキルのスキルタイプごとのアイコン番号
  ACTOR_ICON_SKILL  = [122, 117, 16, 16, 16, 16, 16, 16]
  # ※ACTOR_ICONが true の時のみ
  #   [通常攻撃, 防御, アイテム, 逃走] のアイコン番号
  ACTOR_ICON_OTHER = [116, 121, 260, 467]
  # ※ACTOR_ICONが true の時のみ
  ...
  PARTY_ICON_LIST = [122, 467, 16, 16, 16, 16]
...
class Window_ActorCommand < Window_Command
  ...
  def add_CW_party_command
    unless $cwinter_script_battle_formation
      text  = "エラー「戦闘コマンドカスタム」\n\n"
      text += "元スクリプト「CW戦闘中パーティ編成」が導入されていません\n\n"
      text += "元スクリプト「CW戦闘中パーティ編成」を導入するか\n"
  ...
  def add_DA_party_command
    unless BattleCommand_Custom.ex_DA_party?
      text  = "エラー「戦闘コマンドカスタム」\n\n"
      text += "拡張スクリプト「DAアクターコマンドPT編成」が導入されていません\n\n"
      text += "拡張スクリプト「DAアクターコマンドPT編成」を導入するか\n"
